...
int main() {
	...
	while (!inFile.eof()) {
		...
		if (opCode == 'E') {
			inFile >> WRCode >> writeValue;//ASSUMING THERE IS A DUMMY VARIABLE AFTER 'R' SINCE NO EXAMPLES WITH 'R' WERE GIVEN IN THE SAMPLE INPUT
			if (WRCode == 'W') {
				if (validAddress) {
